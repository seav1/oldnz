...
EOF
  ...
  wget -O $WORK_DIR/nezha-agent.zip ${GH_PROXY}https://github.com/nezhahq/agent/releases/download/v0.17.6/nezha-agent_linux_$ARCH.zip
...
[program:agent]
command=$WORK_DIR/nezha-agent -s localhost:$GRPC_PORT -p --disable-auto-update --disable-force-update $LOCAL_TOKEN
autostart=true
autorestart=true
stderr_logfile=/dev/null
stdout_logfile=/dev/null
