...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  [ -n "$GH_PROXY" ] && wget --server-response --quiet --output-document=/dev/null --no-check-certificate --tries=2 --timeout=3 https://raw.githubusercontent.com/seav1/oldnz/main/README.md >/dev/null 2>&1 && unset GH_PROXY
  ...
  if [ "$REVERSE_PROXY_MODE" = 'grpcwebproxy' ]; then
    wget -c ${GH_PROXY}https://github.com/seav1/oldnz/releases/download/grpcwebproxy/grpcwebproxy-linux-$ARCH.tar.gz -qO- | tar xz -C $WORK_DIR
...
EOF
  ...
  # 下载需要的应用
  wget -O /tmp/dashboard.zip ${GH_PROXY}https://github.com/naiba/nezha/releases/download/v0.16.5/dashboard-linux-$ARCH.zip
  ...
  wget -O $WORK_DIR/nezha-agent.zip ${GH_PROXY}https://github.com/nezhahq/agent/releases/download/v0.16.2/nezha-agent_linux_$ARCH.zip
...
EOF
  ...
  wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/seav1/oldnz/raw/main/sqlite.db
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/seav1/oldnz/main/template/backup.sh | sed '1,/^########/d' >> $WORK_DIR/backup.sh
...
EOF
    ...
    wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/seav1/oldnz/main/template/restore.sh | sed '1,/^########/d' >> $WORK_DIR/restore.sh
  fi
...
EOF
  ...
  wget -qO- ${GH_PROXY}https://raw.githubusercontent.com/seav1/oldnz/main/template/renew.sh | sed '1,/^########/d' >> $WORK_DIR/renew.sh
